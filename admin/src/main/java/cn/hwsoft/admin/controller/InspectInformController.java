...
public class InspectInformController {
    ...
    public JSONResult selectById(@PathVariable("id") int id){
        ...
        Inspect_type type = typeService.selectById(inform.getType_id());
    ...
    public JSONResult updateProcessStatus(int id, int type, HttpSession session){
        Inspect_inform inform = new Inspect_inform();
        Admin admin = (Admin) session.getAttribute("admin");
        inform.setId(id);
        inform.setProcess_status((byte)type);
        inform.setAdmin_id(admin.getId());
        informService.updateProcessStatus(inform);
        JSONResult result = new JSONResult();
        return result;
    }
     ...
     * @return
     */
    @AuthorityCheck("inform_download")
    @PostMapping("/load")
    @ResponseBody
    ...
    private String writeFile(int id){
        ...
        if(inform.getId()!=null){
            user = userService.selectUser(inform.getUser_id());
        }
        if(inform.getType_id()!=null){
            type = typeService.selectById(inform.getType_id());
        }
        String word = FileUtil.informWord(inform,user,type);
        images.add(word);
        //保存文件为zip
        String zipFile = FileUtil.zip(images,inform.getReceipt(),null);
        return  zipFile;
    }
